...
function AgentCard({ agent, index, settings }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="p-6">
        ...
        <p className="text-sm font-semibold mt-2" style={{ color: settings.primaryColor }}>
          {agent.dealsCount} Deals Closed
        </p>
        <button className="mt-4 w-full border-2 px-5 py-2.5 rounded-lg text-sm font-medium hover:text-white hover:brightness-110 transition-colors" style={{ borderColor: settings.primaryColor, color: settings.primaryColor }}>
          Contact
        </button>
      </div>
    </motion.div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="font-serif text-3xl sm:text-4xl font-bold text-gray-900">
            Meet Our <span style={{ color: settings.primaryColor }}>Agents</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {mockAgents.map((agent, index) => (
            <AgentCard key={agent.id} agent={agent} index={index} settings={settings} />
          ))}
        </div>
      </div>
    </section>
